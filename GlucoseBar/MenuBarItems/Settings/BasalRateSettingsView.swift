...
struct BasalRateSettingsView: View {
    ...
    init(viewSettings: MenuBarItemContainer?) {
        if let viewSettings = viewSettings {
            ...
            let container = MenuBarItemContainer(type: .basalrate)
            ...
            if showIcon != .iconColorHidden {
                GridRow {
                    ...
                    Picker("", selection: $iconPlacement) {
                        Text("Before Basal Value").tag(settingValue.iconPlacementLeading)
                        Text("After Basal Value").tag(settingValue.iconPlacementTrailing)
